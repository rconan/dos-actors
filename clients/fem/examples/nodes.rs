...
fn main() -> anyhow::Result<()> {
    ...
    let io_name = String::from("MC_M2_SmHex_D");
    let get_out = Box::<dyn fem_io::GetOut>::try_from(io_name.clone())?;
    ...
    let mut file = File::create(format!("{}_nodes.pkl", io_name.to_lowercase()))?;
    serde_pickle::to_writer(&mut file, &xyz, Default::default())?;
