/*!
# M2 assembly RBMs command integrated test

Testing that commands applied to the M2 segment positioners, given in rigid body displacement, results in the actual rigid body displacement of the segments.

Test all the rigid bodies motion for all given segment at once.

The tests fail if any rigid body displacement is more than 1% of the input command.

## Usage
```shell
MOUNT_MODEL=MOUNT_FDR_1kHz cargo test -r --features cuda --test m2_rbms -- --show-output
```
...
use gmt_dos_clients_servos::{GmtFem, GmtM2Hex, GmtServoMechanisms};
use gmt_fem::FEM;
use interface::{
    Tick,
    optics::{
...
async fn main() -> anyhow::Result<()> {
    ...
    let rbms: Vec<_> = (0..7)
        ...
        .collect();
    let optical_state =
        OpticalState::default().zero_point(OpticalState::m2(MirrorState::from_rbms(&rbms)));
    ...
    actorscript!(
        1: timer[Tick] -> {servos::GmtFem}[M2RigidBodyMotions]!$
        1: optical_state[M2State] -> {servos::GmtM2Hex}
    );

    let data: Vec<f64> = model_logging_1
        .into_inner()
        .unwrap()
        .into_iter(format!("M2RigidBodyMotions"))?
        .last()
        .unwrap();
